...
(function () {
	...
	document.body.addEventListener("click", async function (e) {
		...
		const url = `/dashboard/customer/${id}/edit/`; // đúng với urls.py của bạn
		...
		// Lưu reference tới action
		const form = editContent.querySelector("#edit-form");
		currentSaveUrl = form ? form.getAttribute("data-action") : null;

		// Gắn handler nút Save trong modal này
		const btnSave =
			editContent.querySelector("#btn-save-customer") ||
	...
	async function onSaveClick() {
		...
		if (data.ok) {
			...
			editModal.hide();
			// Có thể refresh bảng hoặc hiển thị toast
			location.reload();
			return;
		}
